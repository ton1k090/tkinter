# Калькулятор с использованием библиотеки tkinter
...
def add_operation(operation):
    '''Функция добавления операции над числами'''
    value = calc.get()
    if value[-1] in '+-/*':
        value = value[:-1]
...
def calculate():
    '''Функция выполняющая операции над числами'''
    value = calc.get()
    if value[-1] in '+-/*':
        value = value + value[:-1]
    calc.delete(0, tk.END)
...
def clear():
    '''Функция очистить окно'''
    calc.delete(0, tk.END)
    calc.insert(0, 0)


def make_digit_button(digit):
    '''Функция параметров кнопок с числами'''
    return tk.Button(text=digit, bd=5, font=('Arial', 13), command=lambda: add_digit(digit))


def make_operation_button(operation):
    '''Функция параметров кнопок с операциями'''
    return tk.Button(text=operation, bd=5, font=('Arial', 13), fg='red',
                     command=lambda: add_operation(operation))


def make_calc_button(operation):
    '''Функция параметров кнопки действий над числами'''
    return tk.Button(text=operation, bd=5, font=('Arial', 13), fg='red',
                     command=calculate)


def make_clear_button(operation):
    '''Функция параметров кнопки очистки окна'''
    return tk.Button(text=operation, bd=5, font=('Arial', 13), fg='red',
                     command=clear)


def press_key(event):
    '''Функция для кнопки нажатия с клавиатуры'''
    print(event.char)
...
calc.insert(0, '0')
calc.grid(row=0, column=0, columnspan=4, sticky='we', padx=4)
